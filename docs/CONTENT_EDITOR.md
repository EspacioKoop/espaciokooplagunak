...
El formulario ya ofrece verticales GUI tipadas para salud de sistemas, recursos y
carga: selector canónico de sistema, IDs seguros, salud/cantidad validadas,
aplicar/quitar override y undo/redo compartido sobre el staging. Los overrides de
puestos todavía se conservan pero no se exponen. El documento no toca el ECS. La
sesión C++ pura prepara todos esos cambios con dirty state, historial acotado y
rollback al último snapshot guardado. Previsualización y aplicación al mundo se
