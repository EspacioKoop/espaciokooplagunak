...
# Import the Wikidata client from the tools/apis package
from .apis import wikidata
# We need to access the ULTIMO_MOTIVO from the core to check errors
...
def pedir_a_wikidata():
    ...
    # Use the wikidata client from tools/apis
    result = wikidata.wikidata(SPARQL_QUERY)
    if result is None:
        # Por que fallo
        if ULTIMO_MOTIVO == 'presupuesto':
            raise RuntimeError('Presupuesto diario agotado para Wikidata')
        elif ULTIMO_MOTIVO == 'no_encontrado':
            raise RuntimeError('No se obtuvo respuesta de Wikidata')
        elif ULTIMO_MOTIVO == 'cache_fallo':
...
def main():
    ...
    parser.add_argument('--desde-fichero', help='Leer la respuesta cruda la respuesta cruda de un archivo en lugar de hacer la petición a Wikidata.')
