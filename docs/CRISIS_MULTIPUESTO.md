...
Suites del puente (pytest) y del módulo (Node) en verde en local; la sintaxis Lua la verifica el job
`LuaTest` de CI (`luac -p` sobre `scripts/`), no se comprobó en la máquina de desarrollo. **Falta el playtest
