...
export function caja([cx, cy, cz], [ancho, alto, fondo]) {
  ...
  return { vertices, caras, uvs: uvsDeCaja(ancho, alto, fondo) };
}
 ...
 */
export function uvsDeCaja(ancho, alto, fondo) {
  const u = (metros) => Math.abs(metros) / METROS_POR_TEXTURA;
...
export function prisma(
  ...
  const paso = perimetro / lados / METROS_POR_TEXTURA;
  const v1 = Math.abs(alto) / METROS_POR_TEXTURA;
  const uvs = [];
  for (let j = 0; j < lados; j += 1) {
    const u0 = j * paso;
    uvs.push([
  ...
  const lado = ((radioArriba * 2) / METROS_POR_TEXTURA) || 1;
