...
/**
 * Estampa la textura de una cara DENTRO de ella, recortando con el propio
 * polígono. Es la técnica de la época y no un apaño: la PSX no tenía potencia
 * para texturas por píxel con corrección de perspectiva, así que el detalle de
 * superficie se resolvía con patrones planos pegados a la cara — y el resultado
 * es justo el que se recuerda.
 *
 * Se hace aquí y no en `retro3d.mjs` por la misma razón por la que el patrón
 * viaja desde la sala: el motor recibe una malla y un color, y meterle texturas
 * sería convertir un rasterizador de cascos en un motor de escenarios.
 *
 * Los patrones se dibujan en el ESPACIO DE PANTALLA, no en el de la cara. Es
 * incorrecto en cuanto la cara se inclina —las juntas no convergen hacia el
 * punto de fuga— y es exactamente lo que hacía una consola sin corrección de
 * perspectiva. Perseguir la corrección aquí sería más caro y menos fiel.
 */
export function estamparCara(ctx, poligono) {
  const patron = poligono?.patron;
  const puntos = poligono?.puntos;
  if (!patron || patron === "liso" || !Array.isArray(puntos) || puntos.length < 3) return 0;
  if (typeof ctx?.save !== "function" || typeof ctx?.clip !== "function") return 0;
  ...
  // Una cara de menos de tres píxeles no admite textura: solo añadiría ruido y
  // se llevaría un recorte entero por delante.
  ...
  let trazos = 0;
  if (patron === "plancha") {
    // Planchas de suelo: juntas anchas en un sentido y remaches en el otro.
    ctx.fillStyle = velo(CANTINA.sombra, 0.22);
    ...
    // Madera: líneas finas y juntas, sin retícula. Si se cruzaran, sería tela.
    ctx.fillStyle = velo(CANTINA.sombra, 0.14);
    for (let y = y0; y <= y1; y += 3) {
      ctx.fillRect(x0, y, x1 - x0, 1);
      trazos += 1;
    }
  } else if (patron === "rejilla") {
    // Chapa de mamparo: retícula corta, que es lo que la separa de la madera.
    ctx.fillStyle = velo(CANTINA.sombra, 0.16);
...
/** Estampa todas las caras de una escena, en su mismo orden de pintado. */
export function estamparEscena(ctx, escena) {
  if (!ctx || !Array.isArray(escena?.poligonos)) return 0;
  let caras = 0;
  for (const poligono of escena.poligonos) {
    if (estamparCara(ctx, poligono) > 0) caras += 1;
  }
  return caras;
}
