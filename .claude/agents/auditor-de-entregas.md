...
2. **Nada encoge.** Suites fichero a fichero —nunca en total, que puede subir mientras se
   pierden las pruebas buenas— y `tools/validate_es_locale.py .` para los catálogos, que
   compara clave a clave contra el fuente y canta `key mismatch` con las dos cifras.
