...
test("el catálogo tiene hoy las tres mesas: póker, dados y blackjack", () => {
  const puertas = puertasCantina();
  assert.deepEqual(puertas.map((p) => p.id), ["poker", "dados", "blackjack"]);
...
test("puertaPorId encuentra la puerta existente y no inventa una para ids ajenos", () => {
  ...
  assert.equal(puertaPorId("blackjack")?.id, "blackjack");
